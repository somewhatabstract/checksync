...
describe("#parseFile", () => {
    ...
    it("should provide path normalization function", async () => {
        // Arrange
        const mockFile = mockFakeFile();
        mockFile.push(null); // <- End of file
        const markerParserSpy = jest.spyOn(MarkerParser, "default");
        ...
        const normalizationCb = markerParserSpy.mock.calls[0][0];
        const result = normalizationCb("targetfile.js");
        await promise;
        ...
        expect(pathDirnameSpy).toHaveBeenCalledWith("file.js");
    });
    ...
    it("should output error group if markers not terminated", async () => {
        ...
        expect(groupSpy).toHaveBeenCalledWith(
            " ERROR  These markers are not terminated",
        );
